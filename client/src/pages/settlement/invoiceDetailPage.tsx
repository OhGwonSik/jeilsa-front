...
// URL 파라미터의 인터페이스를 정의합니다.
interface InvoiceUrlParams {
    billId?: string;
    billDtlId?: string;
    [key: string]: string | undefined; // 모든 문자열 키 허용
}

// InvoiceTable 컴포넌트를 직접 정의합니다.
...
// InvoiceDetailPage 컴포넌트입니다.
const InvoiceDetailPage = () => {
    // URL에서 billId와 billDtlId를 가져옵니다.
    const { billId, billDtlId } = useParams<InvoiceUrlParams>();
    ...
    useEffect(() => {
        const fetchInvoiceData = async () => {
            ...
            try {
                // 토큰이 없는 모든 사람들 접근 가능
                const response = await axios.get<CompanyInvoiceDTO>(`/api/bill-print/page/company-invoice/${billId}/${billDtlId}`);
                console.log(response)
                if(response.data?.shipmentInfoList){
                    setData(response.data);
                }else{
                    setData(getEmptyData());
                }
            } catch (err) {
                console.error("청구서 데이터 가져오기 실패:", err);
    ...
    // 데이터가 null일 경우 (API 호출 실패 시) 빈 데이터를 렌더링하도록
    if (!data) {
      setData(getEmptyData());
    }
                        ...
                        <tbody>
                            <tr>
                                <td colSpan={8} style={{ textAlign: "center", padding: "5px", fontSize: "13px", fontWeight: "bold", border: "1px solid #000" }}>
                                    {data?.billCompanyInfo.billCompanyNm} {data?.month}월 택배 청구 내역서
                                </td>
                            </tr>
                            <tr>
